...
.select("*, cases(current_stage, leads(full_name)), profiles!estimations_approved_by_fkey(full_name)")  .order("created_at", { ascending: false }),
      supabase
        .from("cases")
...
supabase.from("client_intake")
  ...
  // Auto-link intake when case selected
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast({ title: "Estimation created", description: "Agent will be notified once approved." });
      setShowCreate(false);
  ...
  const handleApprove = async (estimation: any) => {
    ...
    // Advance case stage to Estimation Approved
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        title="Estimations"
        description="Review and approve filing fee estimates"
        actions={
          isAdmin ? (
              ...
              <DialogContent className="max-w-lg">
                ...
                <form onSubmit={handleCreate} className="space-y-4 pt-2">
                  ...
                  <Button
                    type="submit"
                    className="w-full"
                    disabled={creating || !form.estimated_fee_usd}
                  >
        ...
        {filtered.length > 0 && (
          <div className="flex items-center justify-between border-t bg-muted/10 px-4 py-2">
            <p className="text-xs text-muted-foreground">
              {filtered.length} of {estimations.length} estimations
            </p>
            <p className="text-xs font-semibold text-foreground">
              Total Approved: $
              {estimations
                .filter((e) => e.status === "approved")
                .reduce((s, e) => s + Number(e.estimated_fee_usd), 0)
            ...
            <div className="space-y-4 pt-2 text-sm">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="font-medium">{viewItem.cases?.leads?.full_name || "—"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-foreground">
                    ${Number(viewItem.estimated_fee_usd).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Completion</p>
                  <p className="font-medium">
                    {viewItem.estimated_completion_days
                      ? `${viewItem.estimated_completion_days} days`
                      : "—"}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Case Stage</p>
                  <p>{viewItem.cases?.current_stage || "—"}</p>
                </div>
                <div>
                ...
                <div className="flex items-center gap-2 rounded-lg bg-green-50 border border-green-200 p-3">
                  ...
                  <div>
                    <p className="text-xs font-medium text-green-800">
                      Approved by {viewItem.profiles?.full_name || "Admin"}
                    </p>
                    <p className="text-xs text-green-700">
                      {viewItem.approved_at
                        ? new Date(viewItem.approved_at).toLocaleString()
                        : ""}
                    </p>
                  </div>
                </div>
          ...
          <DialogContent className="max-w-sm">
            ...
            <div className="space-y-4 pt-2">
              <p className="text-sm text-muted-foreground">
                Rejecting estimation for{" "}
                <span className="font-medium text-foreground">
                  {rejectDialog.cases?.leads?.full_name || "this client"}
                </span>. Provide a reason so the agent can revise.
              </p>
              <div className="space-y-2">
                <Label>Rejection Reason</Label>
                <Textarea
                  rows={3}
                  placeholder="e.g. Fee too high, needs re-review..."
                  value={rejectReason}
